...
from app.models import User, PartnerUser, WebhookEvent
...
class PostgresDispatcher(Dispatcher):
    def send(self, event: bytes):
        instance = WebhookEvent.create(content=event, flush=True)
...
class EventDispatcher:
    ...
    def send_event(
        ...
        event = event_pb2.Event(
            user_id=user.id, external_user_id="EXTERNAL", partner_id=1, content=content
        )
