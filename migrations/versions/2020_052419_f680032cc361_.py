...
def upgrade():
    ...
    op.create_index(
        op.f("ix_fido_credential_id"), "fido", ["credential_id"], unique=True
    )
...
def downgrade():
    ...
    op.add_column(
        "users",
        sa.Column(
            "fido_credential_id", sa.VARCHAR(), autoincrement=False, nullable=True
        ),
    )
    op.add_column(
        "users", sa.Column("fido_pk", sa.VARCHAR(), autoincrement=False, nullable=True)
    )
    op.add_column(
        "users",
        sa.Column("fido_sign_count", sa.INTEGER(), autoincrement=False, nullable=True),
    )
    op.create_unique_constraint("users_fido_pk_key", "users", ["fido_pk"])
    op.create_unique_constraint(
        "users_fido_credential_id_key", "users", ["fido_credential_id"]
    )
