...
2. upgrade to the latest version which is 4.6.4-beta
...
## Upgrade to the latest version 4.6.4-beta

```bash
# Pull the latest version
sudo docker pull simplelogin/app:4.6.4-beta
...
sudo docker run --rm \
    ...
    --network="sl-network" \
    simplelogin/app:4.6.4-beta alembic upgrade head
...
sudo docker run --rm \
    ...
    -v $(pwd)/dkim.pub.key:/dkim.pub.key \
    --network="sl-network" \
    simplelogin/app:4.6.4-beta python init_app.py
...
sudo docker run -d \
    ...
    --restart always \
    --network="sl-network" \
    simplelogin/app:4.6.4-beta python email_handler.py
...
docker run -d \
    ...
    --restart always \
    --network="sl-network" \
    simplelogin/app:4.6.4-beta python job_runner.py
